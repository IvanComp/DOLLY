...
public class Model {
	...
	// Get intermediate state ids for a given object and fsm
    public ArrayList<Integer> getIntermediateStateIds(int i, int j){
    	return modelObjects.get(i).getIntermediateStateIds(j);
    }


	 // Get ending state ids for a given object and fsm
	 public ArrayList<Integer> getEndingStatesIds(int i, int j){
		return modelObjects.get(i).getEndingStatesIds(j);
    ...
    public int getMethodId(int eventID, int objectID) {
		for (Method method : modelMethods) {
			if (method.ownereventid == eventID && method.ownerobjectid == objectID) {
				return method.id;
			}
		}
		return 0;
	...
	// Return a list of owned event ids, given an object id
    public ArrayList<Integer> getOwnedEventIds(int i){    	
		return modelObjects.get(i).getOwnedEventIds();
    }

	// Return a list of acquired event ids, given an object id
    public ArrayList<Integer> getAcqEventIds(int objectId){    	
		for (int i = 0; i < modelObjects.size(); i++) {	    		
    		if(getObjectId(i)  == objectId){
				return modelObjects.get(i).getAcquiredEventIds();
    		}	
    	}
    	return null;
    }
	...
	//Get target state id for object obj, fsm j, source state and method id
	public int getTargetStateId(int i, int j, int sourceStateId, int eventID){
		int objectID = getObjectId(i);
		for(int tran = 0; tran < getTransitionNumber(i,j); tran++){
			int srcStateId = getTransitionSourceStateId(i, j, tran);
			if (srcStateId == sourceStateId) {
				for(int met=0; met < getMethodAmount(i,j,tran); met++){
					if(getMethodId(eventID, objectID) == getMethodId(i,j,tran,met)){
						int targetStateId = getTransitionTargetStateId(i, j, tran);
