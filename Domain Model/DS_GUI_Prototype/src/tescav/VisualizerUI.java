...
public class VisualizerUI {
	...
	HashMap<String, Boolean> covered = new HashMap<String, Boolean>();
	...
	private void summary() {
		MerodeLogger.logln("["+ Instant.now() + "]" + " Opening TesCaV summary");
	...
	private void tescav() {
		MerodeLogger.logln("["+ Instant.now() + "]" + " Opening TesCaV main page");
		...
		// When subMenuHelp button is pressed, prepare and paint
		subMenuHelp.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				JLabel title = new JLabel(vb.getHelpText(), JLabel.CENTER);
				p.add(title, c);
				frame.add(p);

				// Combobox
				String hintCombo = "<html><font size=+1>" + "Select Object" + "</font></html>";
				final JComboBox<String> criteriaList = vb.getComboBoxCriteria();
				criteriaList.setRenderer(new ComboBoxRenderer(hintCombo));
				criteriaList.setSelectedIndex(-1);
				criteriaList.setPrototypeDisplayValue(hintCombo);
				c.gridx = 0;
				c.gridy = 1;
				c.gridwidth = 1;
				c.weightx = 0.5;
				c.anchor = GridBagConstraints.CENTER;
				p.add(criteriaList, c);

				criteriaList.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						JFrame frame = new JFrame("Detailed help");

						Box container = Box.createVerticalBox();
						JScrollPane scrPane = new JScrollPane(container);
						JPanel textPanel = new JPanel();

						String a = String.valueOf(criteriaList.getSelectedItem());
						String critName = a.substring(a.indexOf("1") + 2);
						int criterionIndex = getCriterionIndex(critName);

						String helpText = getCriterionHelpText(criterionIndex);

						JLabel helpTextLabel = new JLabel(helpText, JLabel.CENTER);		
		...
		// Combobox
		String hintCombo = "<html><font size=+1>" + "Select Object" + "</font></html>";
		final JComboBox<String> classList = vb.getComboBoxClasses();
	...
	private int getCriterionIndex(String critName){
		// For each criterion
		for (int i = 0; i < vb.getCriteriaAmount(); i++) {
			String criterionName = vb.getCriterionName(i);
			if (criterionName.equals(critName)){
				return i;
			}
		}
		return -1;
	...
	private void displayCoverage(final String selectedClass) {
		...
		for (int i = 0; i < vb.getCriteriaAmount(); i++) {
			...
			view.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					MerodeLogger.logln("["+ Instant.now() + "]" + " Opening TesCaV for " + criterion + " for " + selectedClass);
	...
	private String getCriterionHelpText(int criterionIndex){
		return coverage.getCriterionHelpText(criterionIndex);
	}

	private ArrayList<String> sortStateIDs(ArrayList<String> stateIDs) {
		ArrayList<String> sortedStateIDs = new ArrayList<String>();
		for (String stateID : stateIDs){
			if (XMLProperties.getStateType(stateID).equals("INITIAL_STATE")){
				sortedStateIDs.add(0, stateID);
			} else if (XMLProperties.getStateType(stateID).equals("STATE")){
				sortedStateIDs.add(1, stateID);
			} else if (XMLProperties.getStateType(stateID).equals("FINAL_STATE")){
				sortedStateIDs.add(stateID);
			}
		}
		return sortedStateIDs;
	}

	private ArrayList<String> getStateNames(ArrayList<String> stateIDs){
		ArrayList<String> stateNames = new ArrayList<String>();
		for (String stateID : stateIDs) {
			stateNames.add(XMLProperties.getStateNameById(stateID));
		}
		return stateNames;
	}
	...
	private int resizeColumnWidth(JTable table) {
		...
		for (int column = 0; column < table.getColumnCount(); column++) {
			int width = getColumnHeaderWidth(table, column);
	...
	private void paint(String selectedClass, int criterionIndex) {
		...
		float minY = 0;
		Graphics2D g2d = null;
		ArrayList<String> stateIDs = XMLProperties.getStateIDs(selectedClass);
		ArrayList<String> stateNames = XMLProperties.getStateNames(selectedClass);
		ArrayList<String> methodNames = XMLProperties.getMethodNames(selectedClass);

		covered = new HashMap<String, Boolean>();

		Object[][] data = null;
		String[] stateNamesList = null;
		...
		try { // try A
			...
			coverage.getCriterionType(criterionIndex).equals(Consts.EDG)){
				...
				ArrayList<String> stateIDsSorted = sortStateIDs(stateIDs);
				ArrayList<String> stateNamesSorted = getStateNames(stateIDsSorted);
				data = new Object[methodNamesSet.size()][stateIDsSorted.size()+1];

				stateNamesList = new String[stateIDsSorted.size()+1];
				stateNamesList[0] = "Method name/from state";
				int index = 1;
				for (String stateName : stateNamesSorted){
					if (!Arrays.asList(stateNamesList).contains(stateName)){
						stateNamesList[index] = stateName;
						index++;
					}
				...
				for (int tc = 0; tc < coverage.getTestCasesAmount(criterionIndex); tc++) {
					if (selectedClass.equals(coverage.getObjectName(criterionIndex, tc))) {
						...
						for (int e = 0; e < elementAmount; e++) {
							...
							Pair<Integer, Integer> tindex = getIndex(data, stateNamesList, methodName, columnName);
							int trgStateId = coverage.getStateId(criterionIndex, tc, e, 1);
							String trgStateName = model.getStateNameById(trgStateId);


							String testingType = coverage.getTestingType(criterionIndex, tc, e);	
							if (!testingType.equals(Consts.IMPOSSIBLE)){
								covered.put(key, coverage.isElementCovered(criterionIndex, tc, e));
								data[tindex.getKey()][tindex.getValue()] = "<html><font color = 'white'>" + trgStateName + "</font></html>";
				...
				JTable table = new JTable(data, stateNamesList) {
					@Override
					public Component prepareRenderer(TableCellRenderer renderer, int row, int col) {
						...
						String key = methodName.toLowerCase() + "-" + stateName.toLowerCase();
						if (covered.get(key) != null && !covered.get(key)) {
							//Red
							comp.setBackground(new Color( 187, 30, 16));
						} else if (covered.get(key) != null && covered.get(key)) {
							//Green
				...
				int totalWidth = resizeColumnWidth(table);
