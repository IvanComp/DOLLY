...
public abstract class DeviceresultOperationWindow extends JFrame {
    ...
    protected JTextField t_Unit;
    protected JTextField t_Producedby;
    protected JTextField t_Observedproperty;
	protected JFormattedTextField t_Starttime;
	protected JFormattedTextField t_Endtime;

    private void constructAttributePane(Container p) {
        JPanel att_p = new JPanel (new SpringLayout());
        

        JLabel l_Value = new JLabel("Value [Float]: ", JLabel.TRAILING);
        att_p.add(l_Value);
        ...
        JLabel l_Unit = new JLabel("Unit [String]: ", JLabel.TRAILING);
        att_p.add(l_Unit);

        t_Unit = new JTextField(15);
        t_Unit.setMaximumSize(new Dimension(400, 40));
        l_Unit.setLabelFor(t_Unit);
        
        att_p.add(t_Unit);

        JLabel l_Producedby = new JLabel("Producedby [String]: ", JLabel.TRAILING);
        att_p.add(l_Producedby);

        t_Producedby = new JTextField(15);
        t_Producedby.setMaximumSize(new Dimension(400, 40));
        l_Producedby.setLabelFor(t_Producedby);
        
        att_p.add(t_Producedby);

        JLabel l_Observedproperty = new JLabel("Observedproperty [String]: ", JLabel.TRAILING);
        att_p.add(l_Observedproperty);

        t_Observedproperty = new JTextField(15);
        t_Observedproperty.setMaximumSize(new Dimension(400, 40));
        l_Observedproperty.setLabelFor(t_Observedproperty);
        
        att_p.add(t_Observedproperty);
...
//		});
        ...
        JLabel l_Endtime = new JLabel("Endtime [Date/Time]: ", JLabel.TRAILING);
        att_p.add(l_Endtime);

        l_Endtime.setLabelFor(t_Endtime);
        
//        final JDateChooser dc_2 = new JDateChooser();
//        ((JTextField) dc_2.getDateEditor()).setVisible(false);
//        final DateFormat df_2 = new SimpleDateFormat("yyyy-MM-dd");
//        
//		dc_2.addPropertyChangeListener(new PropertyChangeListener() {
//			@Override
//			public void propertyChange(PropertyChangeEvent evt) {
//				try {
//					if (evt.getPropertyName().equals("date")) {
//				        t_Endtime.setText(df_2.format(dc_2.getDate()));
//					}
//				} catch (Exception e) {
//				}
//			}
...
//		});
		//JPanel date_attr_and_calendar_picker_Panel = new JPanel();
		//date_attr_and_calendar_picker_Panel.add(t_Endtime);
		//date_attr_and_calendar_picker_Panel.add(dc_2);
		//att_p.add(date_attr_and_calendar_picker_Panel);

        UIManager.put("JXDatePicker.arrowIcon", new ImageIcon(DeviceresultOperationWindow.class.getResource("calendar.png")));
        final JXDatePicker jxdc_2 = new JXDatePicker();
        //jxdc_2.getEditor().setVisible(false);
        jxdc_2.setPreferredSize(new Dimension(260, 22));
        jxdc_2.getMonthView().setBoxPaddingX(this.getWidth() > 500 ? 12 : 2);
       
        final DateFormat df_2 = new SimpleDateFormat("yyyy-MM-dd");
        t_Endtime = new JFormattedTextField(df_2);
        jxdc_2.addPropertyChangeListener(new PropertyChangeListener() {
			@Override
			public void propertyChange(PropertyChangeEvent evt) {
				try {
					if (evt.getPropertyName().equals("date")) {
				        t_Endtime.setText(df_2.format(jxdc_2.getDate()));
					}
				} catch (Exception e) {
				}
			}
