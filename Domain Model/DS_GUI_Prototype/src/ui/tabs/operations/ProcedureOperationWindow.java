...
public abstract class ProcedureOperationWindow extends JFrame {
    ...
    protected JTextField t_Devicename;
    protected JTextField t_Description;

    private void constructAttributePane(Container p) {
        JPanel att_p = new JPanel (new SpringLayout());
        

        JLabel l_Devicename = new JLabel("Devicename [String]: ", JLabel.TRAILING);
        att_p.add(l_Devicename);

        t_Devicename = new JTextField(15);
        t_Devicename.setMaximumSize(new Dimension(400, 40));
        l_Devicename.setLabelFor(t_Devicename);
        
        att_p.add(t_Devicename);

        JLabel l_Description = new JLabel("Description [Long text]: ", JLabel.TRAILING);
        att_p.add(l_Description);

        t_Description = new JTextField(15);
        t_Description.setMaximumSize(new Dimension(400, 40));
        l_Description.setLabelFor(t_Description);
        
        att_p.add(t_Description);
